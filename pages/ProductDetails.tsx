        ...
        <div className="w-full md:w-2/5 flex flex-col gap-8 md:py-4 reveal-on-scroll">
          <div>
            <div className="flex items-center gap-3 mb-6">
              <span className="bg-brand-yellow px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-black rounded-full shadow-[0_0_10px_rgba(250,204,21,0.4)]">
                În Stoc
              </span>
            ...
            <div className="leading-none">
              <p className="text-3xl font-bold text-neutral-900 tracking-tight">
                {price.toFixed(0)} <span className="text-sm">RON</span>
              </p>
              {hasDiscount && (
                <p className="text-sm text-red-500 line-through font-mono mt-2">
                ...
                {product.colors.map((color) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                      selectedColor === color
                        ? 'border-brand-yellow scale-110 shadow-[0_0_10px_rgba(250,204,21,0.5)]'
          ...
          <div className="space-y-6 reveal-on-scroll" style={{ transitionDelay: '100ms' }}>
            ...
            <p className="text-neutral-600 leading-loose text-lg font-light">{product.description}</p>
          </div>
          ...
          <div className="bg-neutral-50 p-6 border border-neutral-100 rounded-2xl reveal-on-scroll" style={{ transitionDelay: '200ms' }}>
            ...
            <ul className="space-y-4">
              {product.details?.map((detail, index) => (
                <li
                  key={index}
                  className="flex items-center justify-between text-sm text-neutral-700 border-b border-neutral-200 pb-2 last:border-0"
                >
          ...
          <div className="flex flex-col gap-4 mt-4 sticky bottom-11 md:static z-30 reveal-on-scroll" style={{ transitionDelay: '300ms' }}>
            <Button
              fullWidth
              onClick={() => addToCart(product, selectedColor || undefined)}
              className="py-5 text-base"
            >
              Adaugă în Coș — {price.toFixed(0)} RON
            </Button>
