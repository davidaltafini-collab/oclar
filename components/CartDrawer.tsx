  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
        ...
        {/* MODIFICARE 1: 
            Am inclus tot conținutul (produse + form + FOOTER) în acest container cu scroll.
            Astfel, footer-ul nu mai este sticky (fix) jos, ci curge natural după conținut.
        */}
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <form id="checkout-form" className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                {shippingMethod === 'easybox' && (
                  <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                    ...
                    {/* MODIFICARE 2: BLOCK ALB DE 90PX (OVERLAY) 
                        Acest div nu înconjoară widgetul (nu este container), ci stă înainte în flux,
                        dar are o înălțime de 0 și proiectează absolut "masca" peste elementul următor.
                    */}
                    <div className="relative w-full h-0 z-[1000]">
                        <div className="absolute top-[1px] left-[1px] right-[1px] h-[90px] bg-white rounded-t-[11px] pointer-events-none"></div>
                    </div>

                    {/* CONTAINER HARTA - NEREMODIFICAT */}
          ...
          {/* ⭐ FOOTER INTEGRAT ÎN SCROLL 
            Acum este așezat aici, la finalul listei de conținut, 
            deci nu va mai fi "sticky" pe ecran, ci la baza conținutului.
          */}
          {cart.length > 0 && (
            <div className="p-6 border-t border-neutral-100 bg-white shrink-0 mt-4">
