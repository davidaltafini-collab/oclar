  ...
  namespace JSX {
    interface IntrinsicElements {
      ...
      'gmpx-api-loader': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & {
        ref?: any;
        // Nu setăm props direct aici pentru a evita erorile React 19
      };
    }
  }
}
...
export const CartDrawer: React.FC = () => {
  ...
  const normalizeCounty = (googleCounty: string) => {
    ...
    // Elimină diacriticele pentru a se potrivi cu lista ta ROMANIAN_COUNTIES
  ...
  useEffect(() => {
    if (loaderRef.current && apiKey) {
      // Setăm proprietățile direct pe elementul DOM, nu prin JSX
      loaderRef.current.key = apiKey;
      loaderRef.current.libraries = ['places']; // Array corect
  ...
  useEffect(() => {
    ...
    if (picker && step === 'details') {
      const handlePlaceChange = () => {
        ...
        // Fallback: Dacă nu găsim strada exactă, încercăm din adresa formatată
        ...
        setFormData(prev => ({
          ...
          city: city,
          county: county,
          postalCode: postal, // Codul poștal automat
          address: place.formattedAddress
        }));
        
        // Validăm codul poștal dacă există
  ...
  const handleSubmitOrder = async (e?: React.MouseEvent) => {
    ...
    // Pentru curier, codul poștal este obligatoriu (acum extras și de Google)
            ...
            cart.length === 0 ? (
              ...
              <form id="checkout-form" className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                  ...
                  {/* CÂMPURI MANUALE (POPULATE AUTOMAT SAU EDITABILE) */}
                  <div className="grid grid-cols-2 gap-3">
                    {/* JUDEȚ - DROPDOWN */}
