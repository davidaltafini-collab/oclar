  ...
  namespace JSX {
    interface IntrinsicElements {
      'gmpx-place-picker': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & { 
        ...
        country?: string; // React 19 va seta asta ca proprietate dacă există
      };
      'gmpx-api-loader': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & {
        // Nu folosim 'key' aici în JSX pentru că React îl blochează
        ref?: any;
        libraries?: string[] | string; // Definim ca array pentru React 19
      };
    }
  }
}
...
export const CartDrawer: React.FC = () => {
  ...
  // ⭐ Ref pentru loader ca să putem seta manual cheia API
  ...
  // ⭐ UPDATE CHEIE API (FIX PENTRU REACT "EATING" KEY PROP)
  useEffect(() => {
    if (loaderRef.current && apiKey) {
      // Setăm atributul 'key' manual pe elementul DOM
      loaderRef.current.setAttribute('key', apiKey);
    }
  }, [apiKey, step]); // Rulăm când apare loader-ul (la step 'details')
          ...
          {step === 'cart' ? (
            ...
            <form id="checkout-form" className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                ...
                {/* ⭐ FIX LOADER GOOGLE MAPS */}
                {step === 'details' && apiKey && (
                  <gmpx-api-loader 
                    ref={loaderRef} 
                    // ❗ AICI ESTE FIX-UL PENTRU EROAREA "i.join is not a function": Trimitem un array, nu un string.
                    libraries={['places']} 
                  />
                )}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <gmpx-place-picker ref={pickerRef} placeholder="Ex: Strada Libertății 4, București" id="place-picker" country="ro" style={{ width: '100%' }} />
                  </div>
                ...
                <div>
                  ...
                  <input name="details" placeholder="Bloc, Scara, Etaj, Ap, Interfon..." value={formData.details} onChange={(e) => { const val = e.target.value; setFormData(prev => ({ ...prev, details: val, address: `${prev.street_name} Nr. ${prev.street_number}, ${val}` })); }} className="w-full p-3 border border-neutral-200 rounded-lg focus:outline-none focus:border-black transition-colors" />
                </div>
                <div>
                  <input required={shippingMethod === 'courier'} name="postalCode" placeholder={`Cod Poștal ${shippingMethod === 'courier' ? '*' : '(opțional)'}`} value={formData.postalCode} onChange={handleInputChange} maxLength={6} className={`w-full p-3 border rounded-lg focus:outline-none focus:border-black transition-colors text-base md:text-sm ${validationErrors.postalCode ? 'border-red-500' : 'border-neutral-200'}`} />
                  {validationErrors.postalCode && <p className="text-xs text-red-500 mt-1">{validationErrors.postalCode}</p>}
                  <p className="text-xs text-neutral-400 mt-1">{shippingMethod === 'courier' ? 'Obligatoriu pentru livrare curier (6 cifre)' : 'Opțional pentru EasyBox, necesar pentru factură'}</p>
                </div>
              </div>
