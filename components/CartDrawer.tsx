  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
        ...
        {/* MODIFICARE 1: 
            Acest container (scrollContainerRef) conține acum TOT conținutul,
            inclusiv zona de checkout/footer, pentru a elimina comportamentul "sticky".
        */}
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-5">
                ...
                <form id="checkout-form" className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                  ...
                  {shippingMethod === 'easybox' && (
                    <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                      ...
                      {/* MODIFICARE 2: 
                          CONTAINER WIDGET + OVERLAY
                          - Container relative
                          - Overlay absolut (90px) cu textul "ALEGE LOCKER" peste partea de jos a hărții
                      */}
                      <div className="relative w-full rounded-xl overflow-hidden border border-neutral-200" style={{ height: '500px' }}>
                        {/* Widget-ul efectiv */}
                        <div id="ecolet-locker-widget" style={{ width: '100%', height: '100%' }}></div>
                        
                        {/* OVERLAY VISUAL 90px 
                            - Acoperă zona de jos a hărții
                            - Text Galben/Orange, Background Alb
                            - Nu interferează cu click-urile (pointer-events-none pe text/background opțional, 
                              dar aici lăsăm interacțiunea standard a unui footer vizual)
                        */}
                        <div className="absolute bottom-0 left-0 right-0 h-[90px] bg-white z-10 flex items-center justify-center shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
                             <h3 className="text-xl font-black text-[#EAB308] tracking-widest uppercase">ALEGE LOCKER</h3>
                        </div>
                      </div>
          ...
          {/* MODIFICARE 1 (CONTINUARE):
            Footer-ul a fost mutat AICI, în interiorul scroll-ului, la finalul listei/formularului.
          */}
          {cart.length > 0 && (
            <div className="mt-6 pt-6 border-t border-neutral-100 bg-white">
