  ...
  namespace JSX {
    interface IntrinsicElements {
      ...
      'gmpx-api-loader': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & {
        ref?: any;
        version?: string; // Adăugat pentru a specifica versiunea
      };
    }
  }
  interface Window {
    ...
    // Adăugăm google pentru verificare
    google: any;
  }
}
...
export const CartDrawer: React.FC = () => {
  ...
  // ⭐ INITIALIZARE GOOGLE MAPS API (Cu verificare să nu se încarce de 2 ori)
  useEffect(() => {
    ...
    if (!isGoogleLoaded && loaderRef.current && apiKey) {
      ...
      loaderRef.current.version = 'weekly'; // Fix pentru warning-ul deprecation
    }
  }, [apiKey, isCartOpen]);
  ...
  // =================================================================
  // ⭐ WIDGET EASYBOX - INITIALIZARE FĂRĂ RESTRICȚII CSS
  // =================================================================
  useEffect(() => {
    if (shippingMethod === 'easybox' && step === 'details' && isCartOpen) {
      ...
      // 1. Injectare CSS Widget (Original)
  ...
  // Verificăm dacă Google Maps este deja încărcat
  const shouldRenderLoader = isCartOpen && apiKey && (!window.google || !window.google.maps);
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
        ...
        {/* ⬇️ ZONA DE SCROLL (TOT CONȚINUTUL, INCLUSIV FOOTERUL ESTE AICI) */}
          ...
          {step === 'cart' ? (
            cart.length === 0 ? (
              ...
              <form id="checkout-form" className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                {/* ⭐ EASYBOX WIDGET + BANDA ALBĂ 90px CU TEXT GALBEN */}
                {shippingMethod === 'easybox' && (
                  <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                    ...
                    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '12px', border: '1px solid #e5e5e5' }}>
                        ...
                        {/* ⭐ MASCĂ ALBĂ (90px) + TEXT "ALEGE LOCKER" */}
                        <div style={{
                            position: 'absolute',
                            bottom: 0,
                            left: 0,
      ...
      <style>{`
        ...
        /* FĂRĂ CSS CARE SĂ ASCUNDĂ ELEMENTE DIN WIDGET */
