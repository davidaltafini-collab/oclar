  ...
  namespace JSX {
    interface IntrinsicElements {
      ...
      'gmpx-api-loader': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & {
        ref?: any;
        // NU definim props aici pentru că le setăm manual prin ref
      };
    }
  }
}
...
export const CartDrawer: React.FC = () => {
  ...
  // ⭐ INITIALIZARE GOOGLE MAPS API (SOLUȚIA ROBROUSTĂ)
  // Setăm proprietățile direct pe elementul DOM pentru a evita erorile React 19
  useEffect(() => {
    if (loaderRef.current && apiKey) {
      // 1. Setăm cheia API (proprietatea 'key', nu atribut)
      loaderRef.current.key = apiKey;
      // 2. Setăm librăriile ca ARRAY real (evită eroarea i.join is not a function)
      loaderRef.current.libraries = ['places'];
      // 3. (Opțional) Setăm regiunea pentru rezultate mai bune în RO
      loaderRef.current.region = 'RO';
    }
  }, [apiKey, step]); // Rulăm când apare loader-ul
  ...
  useEffect(() => {
    ...
    if (picker && step === 'details') {
      const handlePlaceChange = () => {
        ...
        // Fallback: Dacă nu avem stradă, folosim adresa formatată
