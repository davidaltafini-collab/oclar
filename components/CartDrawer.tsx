...
export const CartDrawer: React.FC = () => {
  ...
  // Eliminăm loaderRef global pentru a evita conflictele, îl setăm direct pe element sau folosim state
  
  // API Key - Asigură-te că este un string valid
  const apiKey = (import.meta.env.VITE_GOOGLE_MAPS_KEY || (window as any).__GOOGLE_MAPS_KEY__ || '') as string;
  ...
  const toNumber = (v: unknown): number => {
    ...
    if (typeof v === 'string') {
      // Corecție pentru formatele europene (1.200,50 vs 1200.50)
  ...
  // ⭐ BLOCARE SCROLL PAGINĂ PRINCIPALĂ
  useEffect(() => {
    if (isCartOpen) {
  ...
  // ⭐ LISTENER PENTRU SELECTARE ADRESĂ (GOOGLE MAPS)
  useEffect(() => {
    let pickerElement: any = null;
    let listenerAdded = false;

    // Folosim o funcție de handler definită stabil
    const handlePlaceChange = () => {
        ...
        addressComponents.forEach((component: any) => {
            ...
            // Fallback pentru oraș dacă locality lipsește (comun în RO sate/comune)
            if (!city && types.includes("administrative_area_level_2")) city = normalizeCity(component.longText);
            if (!city && types.includes("sublocality")) city = normalizeCity(component.longText);
        ...
        // Fallback dacă Google nu parsează strada corect, o luăm din textul formatat
        if (!street && place.formattedAddress) {
           const parts = place.formattedAddress.split(',');
           if (parts.length > 0) street = parts[0].replace(/\d+/g, '').trim(); // Încercăm să scoatem numărul
        }

        const fullAddressLine = `${street || ''} ${number ? `Nr. ${number}` : ''}`.trim();
        ...
        // Curățăm erorile vizuale
        setValidationErrors(prev => {
            const newErrs = { ...prev };
            delete newErrs.city;
            delete newErrs.county;
            delete newErrs.address;
            delete newErrs.postalCode;
            return newErrs;
        });
    };

    // Retry logic safe pentru Web Components care se încarcă asincron
    const attachListener = () => {
        if (pickerRef.current && !listenerAdded) {
            pickerElement = pickerRef.current;
            pickerElement.addEventListener('gmpx-placechange', handlePlaceChange);
            listenerAdded = true;
        } else if (step === 'details' && isCartOpen) {
             // Dacă elementul nu e gata, reîncercăm scurt
            setTimeout(attachListener, 200);
        }
    };

    if (step === 'details' && isCartOpen) {
        attachListener();
    }

    return () => {
        if (pickerElement && listenerAdded) {
  ...
  useEffect(() => {
    if (!isCartOpen) {
      // Mic delay pentru a nu vedea resetarea vizual în timpul animației de exit
  ...
  // Scroll top la schimbarea pasului
  useEffect(() => {
    ...
    if (shippingMethod === 'easybox' && step === 'details' && isCartOpen) {
      ...
      const initWidget = (attempts = 0) => {
        ...
        if (container && isScriptLoaded) {
          ...
          if (formData.city) {
              const streetPart = formData.street_name || '';
              const numberPart = formData.street_number || '';
              if (streetPart) {
          ...
          try {
              window.BPWidget.init(container, {
                ...
                operators: [
                    { operator: 'DPD' },
                    { operator: 'SAMEDAY' }, // Easybox
                    { operator: 'FAN_COURIER' },
                    { operator: 'CARGUS' },
                ],
                // Folosește un alias generic dacă cel specific nu merge, sau lasă-l pe cel din contul tău
                alias: 'ecolet-192872' 
              });
          } catch (err) {
              console.error("Ecolet init error", err);
          }
        } else {
          if (attempts < 20) {
            widgetTimer = setTimeout(() => initWidget(attempts + 1), 300);
          }
        }
      };
  ...
  }, [shippingMethod, step, isCartOpen, formData.city, formData.street_name]); // Dependențe

  const validateField = (name: string, value: string) => {
    const errors = { ...validationErrors };
    if (name === 'phone') {
      const phoneRegex = /^(\+4|)?(07[0-8]{1}[0-9]{1}|02[0-9]{2}|03[0-9]{2}){1}?(\s|\.|\-)?([0-9]{3}(\s|\.|\-|)){2}$/; // Regex RO mai permisiv
      const cleanVal = value.replace(/\s|-/g, '');
      if (value && cleanVal.length < 10) errors.phone = 'Număr telefon invalid';
  ...
  const handleSubmitOrder = async (e?: React.MouseEvent) => {
    ...
    if (!formData.county) errors.county = 'Adresa incompletă (Județ lipsă)';
    if (!formData.city) errors.city = 'Adresa incompletă (Oraș lipsă)';
    if (!formData.street_name && !formData.address) errors.address = 'Alege o adresă validă';
    ...
    try {
      const finalAddressLine = `${formData.street_name || formData.address} Nr. ${formData.street_number || '-'}, ${formData.details}`.trim();
      ...
      if (paymentMethod === 'card') {
        ...
        else throw new Error('Nu s-a primit URL de checkout');
      } else {
  ...
  const finalTotal = Math.max(0, totalBeforeDiscount - discountAmount); // Nu permitem total negativ

  return (
    <>
      {/* ⭐ GOOGLE LOADER - Plasat aici pentru a folosi environment variable */}
      {isCartOpen && apiKey && (
        <gmpx-api-loader key="gmaps-loader" key-attr={apiKey} region="RO" libraries="places" />
      )}
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
                ...
                {cart.map((item) => (
                  <div key={item.id} className="flex gap-4 p-4 bg-white rounded-xl border border-neutral-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-neutral-500 mt-1">{toNumber(item.price).toFixed(2)} RON</p>
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="w-7 h-7 flex items-center justify-center rounded-full bg-neutral-100 hover:bg-neutral-200 text-sm font-bold">−</button>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                  ...
                  <div className="mb-2 relative z-10">
                      ...
                      {(validationErrors.city || validationErrors.address || validationErrors.county) && (
                          <div className="bg-red-50 border border-red-100 text-red-600 p-2 mt-2 text-xs rounded font-bold animate-pulse">
                              Selectează o adresă validă din lista Google!
                          </div>
                      )}
                  </div>

                  {/* Informații adresă Read Only */}
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 text-xs text-gray-700 grid grid-cols-12 gap-3 items-center">
                      ...
                      <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-3 mb-2 animate-pulse-once">
                        <div className="flex justify-between items-center">
                            <div className="overflow-hidden">
                                ...
                                <p className="text-sm font-bold text-black truncate">{selectedLocker.lockerName}</p>
                                <p className="text-xs text-neutral-600 truncate">{selectedLocker.address}</p>
                            </div>
                            <div className="text-right bg-white px-2 py-1 ml-2 rounded border border-black shadow-sm shrink-0">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <label className={`relative flex items-center gap-4 p-4 border rounded-xl cursor-pointer transition-all duration-200 ${paymentMethod === 'card' ? 'border-black bg-neutral-50 shadow-inner' : 'border-neutral-200 hover:border-neutral-300 hover:bg-neutral-50'}`}>
                      ...
                      <div>
                        ...
                        <span className="text-xs text-neutral-500">Securizat prin Stripe / Netopia</span>
                      </div>
                    </label>
                  </div>
                </div>
        ...
        {/* Footer Complet */}
        {cart.length > 0 && (
          <div className="p-6 border-t border-neutral-100 bg-white shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
      ...
      <style>{`
        ...
        gmpx-place-picker::part(input):focus { 
            ...
            ring: 2px solid black !important;
        }
